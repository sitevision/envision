...
Use input elements with class `.env-range` and `[type="range"]`.

```html
<div class="env-form-field">
   <label for="range-1" class="env-form-label">Range element</label>
   <input class="env-range" type="range" id="range-1" />
</div>
```

To display step markers below the input, add a datalist element.
Connect the datalist element to the input element with the `list` attribute.
...
All elements with classname `.env-range` and `[type="range"]`
will be automatically initialized on `DOMContentLoaded`.

Range elements added to the DOM after document was loaded must be initialized from script.
